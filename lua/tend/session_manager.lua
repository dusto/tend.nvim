...
local ChatHistory = require("tend.ui.chat_history")
...
local SlashCommands = require("tend.acp.slash_commands")

--- @class tend._SessionManagerPrivate
local P = {}
...
--- @param data tend.UserConfig.CreateSessionResponseData | tend.UserConfig.PromptSubmitData | tend.UserConfig.ResponseCompleteData | tend.UserConfig.SessionUpdateData | tend.UserConfig.FileEditData | tend.UserConfig.RequestPermissionData
...
--- @class tend.SessionManager
--- @field session_id? string
--- @field tab_page_id integer
--- @field _is_first_message boolean
--- @field is_generating boolean
--- @field widget tend.ui.ChatWidget
--- @field agent tend.acp.ACPClient
--- @field message_writer tend.ui.MessageWriter
--- @field permission_manager tend.ui.PermissionManager
--- @field status_animation tend.ui.StatusAnimation
--- @field file_list tend.ui.FileList
--- @field code_selection tend.ui.CodeSelection
--- @field diagnostics_list tend.ui.DiagnosticsList
--- @field config_options tend.acp.AgentConfigOptions
--- @field todo_list tend.ui.TodoList
--- @field chat_history tend.ui.ChatHistory
--- @field history_to_send tend.ui.ChatHistory.Message[]|nil
...
function SessionManager:new(tab_page_id)
    local AgentInstance = require("tend.acp.agent_instance")
    ...
    local PermissionManager = require("tend.ui.permission_manager")
    local StatusAnimation = require("tend.ui.status_animation")
    local TodoList = require("tend.ui.todo_list")
    local AgentConfigOptions = require("tend.acp.agent_config_options")
...
--- already exists.
--- @param callback fun(session: tend.SessionManager)
...
function SessionManager:_on_session_update(update)
    ...
    --- @type tend.UserConfig.SessionUpdateData
    local hook_data = {
...
--- @param tool_call tend.ui.MessageWriter.ToolCallBlock
function SessionManager:_on_tool_call(tool_call)
    ...
    --- @type tend.ui.ChatHistory.ToolCall
...
--- @param tool_call_update tend.ui.MessageWriter.ToolCallBlock
            ...
            then
                ...
                local bufnr = (
                    raw_bufnr ~= -1 and vim.api.nvim_buf_is_loaded(raw_bufnr)
                        ...
                        and raw_bufnr
                    or nil
                --- @type tend.UserConfig.FileEditData
                local hook_data = {
                    filepath = abs_path,
                    session_id = self.session_id,
                    tab_page_id = self.tab_page_id,
    ...
    local function callback(result, err)
        ...
        if err then
            ...
            Logger.notify(
                ...
                vim.log.levels.INFO,
                { title = "Tend Thought Effort Level changed" }
            )
        end
    end
...
function SessionManager:_handle_input_submit(input_text)
    ...
    --- @type tend.ui.ChatHistory.UserMessage
    local user_msg = {
        type = "user",
        text = input_text,
        timestamp = os.time(),
    ...
    --- @type tend.UserConfig.PromptSubmitData
    ...
    self.agent:send_prompt(self.session_id, prompt, function(response, err)
        vim.schedule(function()
            ...
            --- @type tend.UserConfig.ResponseCompleteData
...
--- @return tend.acp.ClientHandlers handlers
function SessionManager:_build_handlers()
    --- @type tend.acp.ClientHandlers
    local handlers = {
...
function SessionManager:new_session(opts)
    ...
    self.agent:create_session(handlers, function(response, err)
        ...
        --- @type tend.UserConfig.CreateSessionResponseData
        local hook_data = {
...
--- @param state tend.Theme.SpinnerState
...
--- @param new_config_options tend.acp.ConfigOption[]
