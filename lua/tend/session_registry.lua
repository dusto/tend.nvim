...
local DefaultConfig = require("tend.config_default")
local ACPHealth = require("tend.acp.acp_health")

--- @class tend.SessionRegistry
--- @field sessions table<integer, tend.SessionManager|nil> Weak map: tab_page_id -> SessionManager instance
local SessionRegistry = {
    sessions = setmetatable({}, { __mode = "v" }),
}

--- @param tab_page_id integer|nil
--- @param callback fun(session: tend.SessionManager)|nil
--- @return tend.SessionManager|nil session valid session instance or nil on failure
function SessionRegistry.get_session_for_tab_page(tab_page_id, callback)
    ...
    if not instance then
        ...
        instance = SessionManager:new(tab_page_id) --[[@as tend.SessionManager|nil]]
...
--- @param tab_page_id integer|nil
--- @return tend.SessionManager|nil
...
--- @param on_selected fun(provider_name: tend.UserConfig.ProviderName|nil) Callback that will be called with the selected provider name, if any
