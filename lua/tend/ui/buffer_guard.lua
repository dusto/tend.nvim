-- lua/tend/ui/buffer_guard.lua
local Logger = require("tend.utils.logger")

--- @class tend.ui.BufferGuard
local BufferGuard = {}

--- @class tend.ui.BufferGuard.Callbacks
...
--- @param cb tend.ui.BufferGuard.Callbacks
local function on_buf_enter(cb)
    ...
    -- (set via vim.w[winid].tend_bufnr at window creation)
    local expected = vim.w[cur_win].tend_bufnr
    ...
    if buf_name ~= "" and buftype ~= "nofile" then
        ...
        vim.w[cur_win].tend_bufnr = new_buf
...
--- @param callbacks tend.ui.BufferGuard.Callbacks
--- @return integer augroup_id Used to detach later
function BufferGuard.attach(callbacks)
    local augroup = vim.api.nvim_create_augroup(
        "TendBufferGuard_" .. tostring(callbacks.tab_page_id),
        { clear = true }
    )
    ...
    vim.api.nvim_create_autocmd("BufEnter", {
        ...
        desc = "Tend: redirect non-widget buffers out of " .. "widget windows",
    })
