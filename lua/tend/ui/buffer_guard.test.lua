-- lua/tend/ui/buffer_guard.test.lua
...
local function create_widget_setup()
    ...
    vim.bo[chat_buf].filetype = "TendChat"

    local input_buf = vim.api.nvim_create_buf(false, true)
    vim.bo[input_buf].buftype = "nofile"
    vim.bo[input_buf].filetype = "TendInput"
    ...
    --- @type tend.ui.ChatWidget.WinNrs
    ...
    vim.w[chat_win].tend_bufnr = chat_buf
    vim.w[input_win].tend_bufnr = input_buf

    --- @type tend.ui.BufferGuard.Callbacks
    local callbacks = {
        tab_page_id = tab,
...
describe("BufferGuard", function()
    ...
    it("redirects the foreign buffer to the editor window", function()
        ...
        assert.equal(buf_in_chat, vim.w[s.wins.chat].tend_bufnr)
...
describe("BufferGuard cursor follow (child)", function()
    ...
    local function setup_widget_in_child()
        ...
        child.lua(
            [[
            local BG = require("tend.ui.buffer_guard")
