...
--- @class tend.Clipboard
local M = {}

--- @class tend.Clipboard.SetupOpts
...
function M.show_clipboard_tool_missing_message()
    ...
    FloatingMessage.show({
        title = " Tend.nvim - Image Clipboard ",
        body = body,
    })
end
...
local function is_dir_writable(dir)
    ...
    local ok, fd = pcall(function()
        local test_file = vim.fs.joinpath(dir, ".tend_write_test")
        return vim.uv.fs_open(test_file, "w", 438) -- 438 = 0666 octal
    end)

    if ok and fd then
        pcall(vim.uv.fs_close, fd)
        pcall(vim.uv.fs_unlink, vim.fs.joinpath(dir, ".tend_write_test"))
        return true
    end
...
function M.paste_image()
    ...
    elseif is_dir_writable(vim.uv.os_tmpdir()) then
        ...
        local tend_cache = vim.fs.joinpath(cache_dir, "tend")

        local ok = FileSystem.mkdirp(tend_cache)
        if ok and is_dir_writable(tend_cache) then
            tmp_dir = tend_cache
        else
...
--- @param opts tend.Clipboard.SetupOpts
