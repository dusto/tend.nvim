...
--- @class tend.ui.DiagnosticsList.Diagnostic : vim.Diagnostic
--- @field file_path string Full file path

--- @class tend.ui.DiagnosticsList
--- @field _diagnostics tend.ui.DiagnosticsList.Diagnostic[]
--- @field _bufnr integer the same buffer number as the ChatWidget's diagnostics buffer
--- @field _on_change fun(diagnosticsList: tend.ui.DiagnosticsList)
...
--- @param on_change fun(diagnosticsList: tend.ui.DiagnosticsList) Callback to trigger when diagnostics list changes (e.g., update header)
--- @return tend.ui.DiagnosticsList
...
--- @param diagnostic tend.ui.DiagnosticsList.Diagnostic|nil
...
--- @param diagnostics tend.ui.DiagnosticsList.Diagnostic[]
...
--- @return tend.ui.DiagnosticsList.Diagnostic[]
...
--- @return tend.ui.DiagnosticsList.Diagnostic[] diagnostics Converted diagnostics
function DiagnosticsList.get_buffer_diagnostics(bufnr, opts)
    ...
    --- @type tend.ui.DiagnosticsList.Diagnostic[]
    local diagnostics = {}

    for _, d in ipairs(vim_diagnostics) do
        --- @type tend.ui.DiagnosticsList.Diagnostic
        local diagnostic = vim.tbl_extend("force", d, { file_path = file_path }) --[[@as tend.ui.DiagnosticsList.Diagnostic]]
...
--- @param bufnr integer|nil If nil, uses current buffer
--- @param opts vim.diagnostic.GetOpts|nil Options passed to vim.diagnostic.get()
--- @return tend.ui.DiagnosticsList.Diagnostic[] diagnostics Diagnostics at the cursor line
function DiagnosticsList.get_diagnostics_at_cursor(bufnr, opts)
    ...
    for _, d in ipairs(vim_diagnostics) do
        ...
        if cursor_line >= d.lnum and cursor_line <= end_lnum then
            --- @type tend.ui.DiagnosticsList.Diagnostic
            local diagnostic =
                vim.tbl_extend("force", d, { file_path = file_path }) --[[@as tend.ui.DiagnosticsList.Diagnostic]]
            table.insert(diagnostics, diagnostic)
        end
    end
