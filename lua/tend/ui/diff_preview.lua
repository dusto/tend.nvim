...
local DiffSplitView = require("tend.ui.diff_split_view")
local FileSystem = require("tend.utils.file_system")
local HunkNavigation = require("tend.ui.hunk_navigation")
...
local function get_diff_bufnr(tabpage)
    return vim.t[tabpage]._tend_diff_preview_bufnr
end
...
local function set_diff_bufnr(tabpage, bufnr)
    vim.t[tabpage]._tend_diff_preview_bufnr = bufnr
end
...
--- @param pairs tend.ui.ToolCallDiff.ChangedPair[]
...
--- @class tend.ui.DiffPreview.ShowOpts
...
--- @param opts tend.ui.DiffPreview.ShowOpts
function M.show_diff(opts)
    ...
    if #diff_blocks > 0 then
        ...
        vim.b[bufnr]._tend_prev_modifiable = vim.bo[bufnr].modifiable
...
function M.clear_diff(buf, is_rejection)
    ...
    if bufnr == -1 and type(buf) == "string" then
        ...
        if smart_bufnr ~= -1 and vim.b[smart_bufnr]._tend_suggestion_for then
            bufnr = smart_bufnr
        end
    end
    ...
    local is_suggestion = vim.b[bufnr]._tend_suggestion_for ~= nil
    ...
    if not is_suggestion then
        local prev_modifiable = vim.b[bufnr]._tend_prev_modifiable
        if prev_modifiable ~= nil then
            vim.bo[bufnr].modifiable = prev_modifiable
            vim.b[bufnr]._tend_prev_modifiable = nil
        end
    end
...
function M.setup_diff_navigation_keymaps(buf_nrs)
    ...
    for _, bufnr in pairs(buf_nrs) do
        BufHelpers.keymap_set(bufnr, "n", diff_keymaps.next_hunk, function()
            ...
            desc = "Go to next hunk - Tend DiffPreview",
        })
        ...
        BufHelpers.keymap_set(bufnr, "n", diff_keymaps.prev_hunk, function()
            ...
            desc = "Go to previous hunk - Tend DiffPreview",
        })
    end
end
...
function M._show_new_file_diff(opts, new_lines)
    ...
    vim.b[bufnr]._tend_suggestion_for = opts.file_path
...
function M.cleanup_suggestion_buffer(file_path)
    ...
    if
        suggestion_bufnr == -1
        or not vim.b[suggestion_bufnr]._tend_suggestion_for
    then
        return
    end
