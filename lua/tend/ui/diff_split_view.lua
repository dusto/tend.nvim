...
--- @class tend.ui.DiffSplitView
local M = {}

--- State for split diff view per tabpage
--- @class tend.ui.DiffSplitView.State
...
--- @return tend.ui.DiffSplitView.State|nil state
local function get_state(tabpage)
    return vim.t[tabpage]._tend_diff_split_state
end

--- Set split state for tabpage
--- @param tabpage number Tabpage ID
--- @param state tend.ui.DiffSplitView.State|nil State to set (nil to clear)
local function set_state(tabpage, state)
    vim.t[tabpage]._tend_diff_split_state = state
end
...
local function open_split_view(abs_path, bufnr, target_winid, modified_lines)
    ...
    if vim.b[bufnr]._tend_prev_modifiable == nil then
        vim.b[bufnr]._tend_prev_modifiable = vim.bo[bufnr].modifiable
    end
    if vim.b[bufnr]._tend_prev_modified == nil then
        vim.b[bufnr]._tend_prev_modified = vim.bo[bufnr].modified
    end
...
function M.clear_split_diff(tabpage)
    ...
    if vim.api.nvim_buf_is_valid(state.original_bufnr) then
        local prev_modifiable =
            vim.b[state.original_bufnr]._tend_prev_modifiable
        local prev_modified = vim.b[state.original_bufnr]._tend_prev_modified

        if prev_modifiable ~= nil then
            vim.bo[state.original_bufnr].modifiable = prev_modifiable
            vim.b[state.original_bufnr]._tend_prev_modifiable = nil
        end

        if prev_modified ~= nil then
            vim.bo[state.original_bufnr].modified = prev_modified
            vim.b[state.original_bufnr]._tend_prev_modified = nil
        end
    end
