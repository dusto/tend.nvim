...
---   options = {
---     disabled_filetypes = {
---       statusline = { 'TendChat', 'TendInput', 'TendCode', 'TendFiles', 'TendDiagnostics' },
---       winbar = { 'TendChat', 'TendInput', 'TendCode', 'TendFiles', 'TendDiagnostics' },
---     }
---   }
--- })
--- ```
...
--- @class tend.ui.WindowDecoration
local WindowDecoration = {}

--- @type tend.ui.ChatWidget.Headers
...
--- @class tend.ui.WindowDecoration.Config
...
--- @param parts tend.ui.ChatWidget.HeaderParts
...
--- @return tend.ui.ChatWidget.Headers
function WindowDecoration.get_headers_state(tab_page_id)
    if vim.t[tab_page_id].tend_headers == nil then
        vim.t[tab_page_id].tend_headers = WINDOW_HEADERS
    end
    return vim.t[tab_page_id].tend_headers
end

--- Sets headers for a tabpage
--- @param tab_page_id integer
--- @param headers tend.ui.ChatWidget.Headers
function WindowDecoration.set_headers_state(tab_page_id, headers)
    if vim.api.nvim_tabpage_is_valid(tab_page_id) then
        vim.t[tab_page_id].tend_headers = headers
    end
end
...
--- @param dynamic_header tend.ui.ChatWidget.HeaderParts Runtime header parts
...
local function resolve_header_text(dynamic_header, window_name)
    ...
    if type(user_header) == "table" then
        local merged = vim.tbl_extend("force", dynamic_header, user_header) --[[@as tend.ui.ChatWidget.HeaderParts]]
...
--- `sessionoptions`) persists tend buffer names; on reopen
