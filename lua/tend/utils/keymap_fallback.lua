...
M.MARKER = "[tend-fallback]"

--- Checks if a mapping belongs to tend (prevents infinite loops)
--- @param mapping table
--- @return boolean
local function is_tend_mapping(mapping)
...
--- Skips mappings created by tend to prevent infinite loops
--- @param mode string
--- @param lhs string
...
function M.get_existing_mapping(mode, lhs)
    ...
    if is_tend_mapping(mapping) then
        ...
        for _, map in ipairs(global_maps) do
            if map.lhs == lhs and not is_tend_mapping(map) then
                return map
            end
        end
        return nil
